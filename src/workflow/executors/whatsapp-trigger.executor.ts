...
export class WhatsAppTriggerExecutor implements INodeExecutor {
  async execute(
    ...
    if (!context?.userId) throw new Error("ExecutionContext requerido para WhatsApp Trigger");
    ...
    throw new Error(
      `WhatsApp Trigger timeout after ${timeout}s - no se recibió ningún mensaje (requiere webhook configurado)`
    );
  }
}
